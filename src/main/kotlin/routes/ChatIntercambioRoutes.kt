...
fun Route.chatIntercambioRoutes() {
    ...
    get("/has-completed-exchange/{uidUsuario}") {
        try {
            val authHeader =
            ...
            val hasCompletedExchange = ChatIntercambioService.hasCompletedExchange(uid, uidUsuario)

            val data = mapOf(
                "status" to "success",
                "message" to hasCompletedExchange
            )
